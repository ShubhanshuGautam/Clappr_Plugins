...
export default class EspxWhiteboard extends UICorePlugin {
	  constructor(core) {
	    ...
	    this.rPos = 0; // replay position(index)
	    this.current; // use this for current things.. like brush and curMedContainertime(todo)
	    this.isBanner = false;
	    this.drawing = false;
	    this.drawn = false; 
	    this.points = [];
	  ...
	  onReplay(progress) {
	  	...
	  	while(this.rPos<this.points.length && progress.current >= this.points[this.rPos].timeEl ){
            this.draw(this.points[this.rPos]);
	  		this.rPos++;
	  	}
	  }
	...
	setupCanvas(){
        ...
        this.current.mode   = 'freeform'; //default
      ...
      // called on every point being added to the array
	  initDrawing(data){ // method name should change to sync_allpts_array
 		...
 		//RM this.redrawCanvas();
  		...
  		// UPDATING rPos-------------- // aligning rPos with current playback time
  		if(this.points && this.points.length>0 && this.points[this.rPos] && this.points[this.rPos].timeEl){ 
	  		while(this.points[this.rPos] && Math.floor(this.curMedContTime) >= Math.floor(this.points[this.rPos].timeEl)) {
	  			this.rPos++;
	  		}
  		}
...
*/
    ...
    //Live Draw on Peer side or while Replay
    draw(obj/*,emit*/){
        var w = this.canvas.width;
	    var h = this.canvas.height;
        ...
        switch(obj.mode){
            case 'freeform':
                this.drawLine(obj.data.x0*w,obj.data.y0*h, obj.data.x1*w,obj.data.y1*h, obj.data.color, obj.data.brush/*, emit*/);
                // erase here --  according to draw style
                window.setTimeout(this.eraseLine.bind(this, obj.data.x0*w, obj.data.y0*h, obj.data.x1*w, obj.data.y1*h),2000);
                break;
            case 'stamp':
                console.log('draw:',obj.mode,obj.data.brush,obj.data);
                this.drawStamp(obj.data.brush,obj.data);
                //this.drawStamp();
                break;
            case 'vector':// not defined yet... drawing draggable vectors maybe?
                break;
            default:
                break;
        }
	 ...
	 drawLine(x0, y0, x1, y1, color, brush, emit){
	 	console.log('drawline brush:',brush);
	 	...
	 	//console.log(x0, y0, x1, y1, color, brush, emit);
	    if (!emit) { return; }
	    var w = this.canvas.width;
	    var h = this.canvas.height;
	...
	drawStar(ctx, x, y, r, p, m, color,doErase)
	{
        //console.log(doErase);
	    this.context.save();
        this.context.globalCompositeOperation = "source-over";
   ...
   // Called by all 3 ways: peer draw, self draw, replay
	drawStamp(brush,data){
        var w = this.canvas.width;
	    var h = this.canvas.height;
		...
		switch(brush){
            //-----------
			case 'star':
                if(data){ // peer draw // radius should be normalized by aspect ratio
                    this.drawStar(this.context, data.x*w, data.y*h, data.r, data.p, data.m, data.color);// emit=false
                    window.setTimeout(this.eraseStar.bind(this, data.x*w, data.y*h, data.r, data.p, data.m, data.color),2000);
                    //erase here with timeout
                }
                else{ // self draw
                    this.drawStar(this.context, this.current.x, this.current.y, r, p, m, this.current.color); //emit=true below
                    window.setTimeout(this.eraseStar.bind(this, this.current.x*w, this.current.y*h, r, p, m, this.current.color),2000);
                    //create and emit obj
                    let json = {
                        timeEl: this.curMedContTime,
                        mode: 'stamp',
    		...
    		case 'circle':
        		...
        		break;
            //-----------
    		default://100, 100, 90, 5, 0.5
        		//this.drawStar(this.context, this.current.x, this.current.y, 20, 5, 0.5, this.current.color);
                console.log('No valid shape :',brush);
		}
	}
    
    
    emit(json){
        this.line.set('linepts', json); // current point sent to server
        this.recordPts.set('allpts', this.points); // ALL PTS array sent to server
    }
	  ...
	  onMouseDown(e){
        ...
        // Stamping
	    if(this.current.mode==='stamp'){
	    	this.drawStamp(this.current.brush);// emit=true
	    	return; // Important
	    }else if(this.current.mode==='freeform'){
            // Regular Draw 
            this.drawing = true;   
        }
	  }
	  ...
	  onMouseMove(e){
	    ...
	    if (!this.drawing) { return; }
          //-------------
        if(this.current.mode==='freeform'){ // here drawing on self canvas
	       this.drawLine(this.current.x, this.current.y, e.clientX, e.clientY, this.current.color,this.current.brush, true);
	       window.setTimeout(this.eraseLine.bind(this, this.current.x, this.current.y, e.clientX, e.clientY),2000); // TODO: Move it to draw
        }else if(this.current.mode==='xyz'){
          // maybe continuous stamping etc...       
        }
	    this.current.x = e.clientX;
	    this.current.y = e.clientY;
	  }
	...
	// KEYBOARD HANDLERS
	  onKeyUp(e) {
    	...
    	if (e.ctrlKey && e.keyCode == 83) { // draw Star
            //this.stamp = true;
            this.current.mode = 'stamp';
    		this.current.brush = 'star';
        	//this.stampShape = 'square'
        	console.log(e,e.ctrlKey,e.keyCode,'mode:'+this.current.mode,'brush:'+this.current.brush);
    	}
      }
	  ...
	  onDrawingEvent(json){ // this function draws in realtime ! Redraw function draws when time occurs !
	  	if(this.firstLoad ) {
	  		this.firstLoad = false;
	  		return;
	  	}
	  	// do not draw if my current playback is quite out of sync(time) from what I just recieved as json data line
	  	if(Math.abs(json.timeEl - this.curMedContTime)>1) {
	  		return;
	  	}
