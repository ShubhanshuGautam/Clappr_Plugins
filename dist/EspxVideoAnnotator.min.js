var EspxVideoAnnotator = (function (clappr,deepstream) {
	'use strict';
	...
	class EspxWhiteboard extends clappr.UICorePlugin {
		...
		setupCanvas() {
			if (this.core.mediaControl.container) {
				this.listenTo(this.core.mediaControl.container, clappr.Events.CONTAINER_TIMEUPDATE, this.onTimeUpdate);
				this.listenTo(this.core.mediaControl.container, clappr.Events.CONTAINER_TIMEUPDATE, this.onReplay);
			}
		...
		initDrawing(data) {
			// method name should change to sync_allpts_array
	 ...
	 */
		...
		draw(obj /*,emit*/) {
			...
			switch (obj.mode) {
				...
				case 'stamp':
					...
					break;
				case 'vector':
					// not defined yet... drawing draggable vectors maybe?
					break;
				default:
					break;
			}
		...
		drawStamp(brush, data) {
			...
			switch (brush) {
				...
				case 'star':
					if (data) {
						// peer draw // radius should be normalized by aspect ratio
		...
		onDrawingEvent(json) {
			// this function draws in realtime ! Redraw function draws when time occurs !
			if (this.firstLoad) {
		...
		closeWB() {
			...
			this.stopListening(this.core.mediaControl.container, clappr.Events.CONTAINER_TIMEUPDATE, this.unListen);
